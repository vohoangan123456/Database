INSERT INTO #Description VALUES ('update procedure [m136_be_GetDocumentFieldsAndRelates], update procedure [m136_GetDocumentFieldsAndRelates] ')
GO
...
BEGIN
	...
	--Get linked to this document from another documents
	SELECT d.iDocumentId, d.strName, r.iPlacementId, mi.strName AS iPlacementName
	FROM m136_tbldocument d 
	INNER JOIN m136_relInfo r 
		ON d.iEntityId = r.iEntityId
	LEFT JOIN m136_tblMetaInfoTemplateRecords mi
		ON mi.iMetaInfoTemplateRecordsId = r.iPlacementId
	WHERE r.iItemId = @DocumentId 
		AND r.iRelationTypeId = 136 
		AND r.iEntityId IN 
			(SELECT iEntityId FROM m136_tblDocument WHERE iLatestApproved = 1)
END
GO
