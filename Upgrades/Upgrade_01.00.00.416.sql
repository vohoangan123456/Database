...
BEGIN
	...
	BEGIN
		...
		WHERE
			...
			AND iItemId IN (			
				SELECT iDocumentId FROM m136_tbldocument d WHERE iHandbookid = @ChapterId AND d.iDeleted = 0 AND d.iLatestApproved = 1
				AND d.dtmPublish <= GETDATE()
			UNION 
				SELECT d.iDocumentId FROM
					m136_relVirtualRelation v
						INNER JOIN m136_tblDocument d 
					ON d.iDocumentId = v.iDocumentId
				WHERE
					v.iHandbookId = @ChapterId AND d.iDeleted = 0 AND d.iLatestApproved = 1
				AND d.dtmPublish <= GETDATE()
			)	
			AND val.RegisterValue is not null
		ORDER BY
			Name ASC, val.RegisterValue
