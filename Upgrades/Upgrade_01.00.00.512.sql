...
BEGIN
	...
	JOIN dbo.m136_tblHandbook h 
		ON ac.iHandbookId = h.iHandbookId
	...
	JOIN (
			SELECT SUM(iReadCount) AS iReadCount, iDocumentId 
			FROM dbo.m136_tblDocument 
			GROUP BY iDocumentId
		) d ON result.DokId = d.iDocumentId
	...
	BEGIN
		...
		FROM @resultTable r
		ORDER BY 
			CASE @IsMostRead WHEN 1 THEN r.MostReadTotal END DESC,
			CASE @IsMostRead WHEN 0 THEN r.MostReadTotal END ASC, 
			CASE @IsMostRead WHEN 1 THEN r.MostReadVersion END DESC,
			CASE @IsMostRead WHEN 0 THEN r.MostReadVersion END ASC
	END
	ELSE
	BEGIN
		SELECT TOP(@NumberReturn) r.*
